...
class CentralQVCritic(nn.Module):
    def __init__(self, scheme, args):
        super(CentralQVCritic, self).__init__()
        ...
        self.fc4 = nn.Linear(input_shape, args.hidden_dim)
        self.fc2 = nn.Linear(args.hidden_dim, args.hidden_dim)
        self.fc3 = nn.Linear(args.hidden_dim, args.hidden_dim)
        self.q = nn.Linear(args.hidden_dim, self.n_actions)     # returns Q-values
        self.v = nn.Linear(args.hidden_dim, 1)                  # returns V   
        return

    def forward(self, batch, t=None, flag="q"):
        inputs, bs, max_t = self._build_inputs(batch, t=t)
        if flag == "q":
            fc1 = F.relu(self.fc1(inputs))
            x = F.relu(self.fc2(fc1))
            return self.q(x)         # shape: (batch_size, maxseqlen, num_agents, num_actions)
        else:
            fc4 = F.relu(self.fc4(inputs))
            x = F.relu(self.fc3(fc4))
            return self.v(x).view(self.args.batch_size, -1, self.n_agents)      # shape: (batch_size, maxseqlen, num_agents)   
