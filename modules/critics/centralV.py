...
class CentralVCritic(nn.Module):
    def __init__(self, scheme, args):
        ...
        self.fc3 = nn.Linear(args.hidden_dim, 1)        # returns single V

    def forward(self, inputs, t=None):
        #inputs, bs, max_t = self._build_inputs(batch, t=t)
        x = F.relu(self.fc1(inputs))
        x = F.relu(self.fc2(x))
        v = self.fc3(x)     # shape: (batch_size, maxseqlen, num_agents, 1)
        return v
    ...
    def _get_input_shape(self, scheme):
        ...
        # belief (if exists in scheme)
        if "belief" in scheme:
            # belief_batch shape: [B, T, N, BeliefDim] or [B, T, BeliefDim]
            # When concatenated, we use the last dimension (feature dimension)
            belief_shape = scheme["belief"]["vshape"]
            if isinstance(belief_shape, (list, tuple)):
                # If belief has multiple dimensions, take the last one (feature dimension)
                belief_dim = belief_shape[-1] if len(belief_shape) > 0 else 0
            else:
                # If belief_shape is a scalar (int), use it directly
                belief_dim = belief_shape
            input_shape += belief_dim
        # agent ID (one-hot)
